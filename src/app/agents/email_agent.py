...
_BASE_INSTRUCTIONS = (
    ...
    "ANGLE OBJECTIVE AND PRIORITY: "
    "The selected angle defines the specific purpose and call to action of this email. "
    "Follow the selected angle's ASK exactly and treat it as higher priority than the generic "
    "win-back objective. "
    "Do not introduce a different sales pitch, behavioral claim, question, or call to action "
    "simply because it may encourage another investment. "
    "The global rules always remain in force across every angle: use only explicitly provided "
    "facts, never invent or infer client details, follow all campaign prohibitions, use only "
    "permitted placeholders, avoid unsupported numbers or financial claims, and maintain a "
    "short, warm, professional, low-pressure tone. Never weaken or override these safety, "
    "factuality, formatting, or JSON output requirements to satisfy an angle. "
    "The angle determines WHAT the email should accomplish. "
    "The shared instructions determine WHAT the email may safely say and HOW it must be "
    "written. "
    "If the angle's ASK conflicts with the generic instruction to encourage the client to "
    "invest again, follow the angle's ASK. If the angle does not support a specific claim or "
    "action, omit it. When in doubt, say less rather than inventing or adding an unsupported "
    "objective."
)
