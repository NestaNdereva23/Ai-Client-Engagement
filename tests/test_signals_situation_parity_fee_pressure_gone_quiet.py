...
from app.agents.situations import recompute_fee_pressure_gone_quiet
from app.agents.watchlist import WatchGroup, WatchlistThresholds, fee_pressure_gone_quiet
...
FUND_ID = 9559
DORMANT_UNDER_PRESSURE_CLIENT = 955901
ACTIVE_UNDER_PRESSURE_CLIENT = 955902
DORMANT_NOT_UNDER_PRESSURE_CLIENT = 955903
NO_RISK_ROW_CLIENT = 955904

CLIENT_IDS = (
    DORMANT_UNDER_PRESSURE_CLIENT,
    ACTIVE_UNDER_PRESSURE_CLIENT,
    DORMANT_NOT_UNDER_PRESSURE_CLIENT,
    NO_RISK_ROW_CLIENT,
)
    ...
    with SessionLocal() as session:
        ...
        session.add_all(
            [
                ...
                _fund(DORMANT_NOT_UNDER_PRESSURE_CLIENT, months_until_empty=None),
        ...
        session.add_all(
            [
                ...
                _risk(DORMANT_NOT_UNDER_PRESSURE_CLIENT, sig_dormant=True),
            ]
        )
        session.commit()
...
def _recompute(session) -> None:
    ...
    recompute_fee_pressure_gone_quiet(session, AS_OF, run.run_id)


def _group() -> WatchGroup:
    with SessionLocal() as session:
        return fee_pressure_gone_quiet(session, THRESHOLDS, AS_OF)


def test_a_dormant_client_under_fee_pressure_is_included(book: None) -> None:
    with SessionLocal() as session:
        _recompute(session)

    group = _group()

    assert (DORMANT_UNDER_PRESSURE_CLIENT, FUND_ID) in _keys(group)


def test_a_client_still_paying_in_is_excluded(book: None) -> None:
    with SessionLocal() as session:
        _recompute(session)

    group = _group()

    assert (ACTIVE_UNDER_PRESSURE_CLIENT, FUND_ID) not in _keys(group)


def test_a_dormant_client_not_under_fee_pressure_is_excluded(book: None) -> None:
    with SessionLocal() as session:
        _recompute(session)

    group = _group()

    assert (DORMANT_NOT_UNDER_PRESSURE_CLIENT, FUND_ID) not in _keys(group)


def test_a_client_with_no_risk_row_is_excluded(book: None) -> None:
    with SessionLocal() as session:
        _recompute(session)
