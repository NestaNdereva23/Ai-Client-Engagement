"""Command line entry point for an active-clients ingestion run.

Run a fresh pull:
    uv run python scripts/active/ingest.py

Resume a run that stopped:
    uv run python scripts/active/ingest.py --run-id <id>

The endpoint is fixed to active-clients. There is a separate, matching script
for the inactive-clients (dormant book) feed at scripts/inactive/ingest.py,
kept apart on purpose: see scripts/inactive/transform.py for why.
"""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Pull the active-clients feed into raw staging.")
